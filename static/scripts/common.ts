...
if (!moduleName) {
    ...
    AliHub.Diagnostics.debugInfo("Labs common library loaded (module name " + moduleName + ").");
    ...
    (<any>tracker).config("url", "http://fgt.labs.desktop");
}
...
function regQrCode() {
    ...
    ko.bindingHandlers["qrcode"] = {
        init: function (element: HTMLElement, valueAccessor: () => any) {
            ...
            (element as any)._qrcode = new QRCode(element, value);
        },
        update: function (element: HTMLElement, valueAccessor: () => any, allBindingsAccessor: KnockoutAllBindingsAccessor, viewModel: any, bindingContext: KnockoutBindingContext) {
            var qrcode = (element as any)._qrcode;
            if (!qrcode) return;
            var value = ko.unwrap(valueAccessor()); 
            if (!value) qrcode.clear();
            else qrcode.makeCode(typeof value === "string" ? value : value.text);
        }
    };
}
...
function regEditor() {
    ...
    ko.bindingHandlers["codeeditor"] = {
        init: function (element: HTMLElement, valueAccessor: () => any) {
            ...
            element.innerHTML = "";
            var value = ko.unwrap(valueAccessor());
            (element as any)._codeEditor = CodeMirror(element, {
                value: codeText,
                lineNumbers: true,
                mode: langM
            });
            (element as any)._codeEditor.setSize(element.style.width || 'auto', element.style.height || 'auto');
        }
    };
}
